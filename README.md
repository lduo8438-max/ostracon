...
| **angular/angular** | **38,278** | **1,697,002** | **44.3** | **239 分鐘** | **3,479 MiB** | **768 MB** |
...
**最後一列是為什麼那個擬合不能外推。** 它對 angular 預測 18.7 分鐘，實測 239 分鐘
——**低估 12.8 倍**。每 revision 的時間從 0.7–1.4 ms 跳到 8.5 ms，峰值記憶體
3.5 GB。這不是線性成長，而且 angular 那一趟**還沒跑到迂迴偵測就異常中止了**，
所以 239 分鐘是下界，不是完整鏈路的數字。
...
| 3.8 萬 commit ／ 170 萬 revision | **超出預算一個數量級**，RSS 3.5 GB，未跑完 |

**如果你的 repo 有數萬個 commit 且宣告密度高，現在不要期待它跑得完。**
這是已知邊界不是暫時的數字，成因（記憶體壓力、非線性成長）還沒定位。
要估自己的 repo：先數 revision（≈ commit 數 × 每次改動碰到的宣告數），
在 25 萬以內可以用上面那個兩項擬合，超過就沒有可靠的外推。
