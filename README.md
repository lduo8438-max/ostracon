...
pnpm test        # 先跑 typecheck，再跑單元測試（237 個）
...
```
111 個曾經存在、後來被整段移除的宣告（A 確證 71｜C 疑似 40）
...
直接排除。`create-t3-app` 實測這道守門擋掉了 **41%（78/189）**的候選：它們是
matcher 漏接的搬移，內容還在 repo 裡，只是換了檔案。少了這道守門，四成的「被推翻」
會是假的，而那種錯誤會讓人相信一段從未發生的歷史。
...
長命分支上，多數 repo 罕見；受影響嚴重的 repo 可以用 `--first-parent` 走訪完全避開，
代價是看不到分支上的個別 commit。
...
把文字證據關聯到 entity，而現有的 evidence 掛在 commit 上而不是 entity 上，
硬接會產生看似有據、實則無關的宣稱。
