...
| **angular/angular** | **38,278** | **2,919,032** | **76.3** | **185 分鐘** | **3,471 MiB** | **1.20 GiB** |
...
**最後一列是為什麼那個擬合不能外推。** 它對 angular 預測 29 分鐘，實測 185 分鐘。
...
**那兩顆 revert 加起來 96 分鐘，佔全程 52%。** 兩顆都是一次重新加回一千多個
檔案的大規模 revert——一個 commit 的候選池就是它碰到的所有檔案的宣告，
而相似度比對隨池子大小超線性成長。

扣掉那兩顆，其餘 38,276 個 commit 是 **89 分鐘 / 1.86 ms per revision**，
而且一般區間只從 4:53 溫和成長到 11:06。**成本不是隨累積資料量等比例惡化的。**
...
| 3.8 萬 commit ／ 292 萬 revision | 185 分鐘、RSS 3.5 GB、DB 1.2 GiB——**跑得完，但要有心理準備** |
| 單一 commit 碰到上千個檔案 | **每顆可能要數十分鐘**，這是目前最大的單點成本 |
