...
選單；直接輸入名稱或路徑會搜尋完整索引，已消亡的宣告逐列標示。每一條時間軸都有固定網址（`#<stable_key>`），
貼給別人打得開。

> **空搜尋是策展入口，輸入文字才跨到完整索引。** `ostracon ui` 先列最多 400 筆
> （Best explained／Most changed），因此打開選單不用搬整份目錄；依名稱或路徑搜尋時，
> 則涵蓋資料庫裡的每一個宣告。實測 pypa/pip 是 21,272 筆、playwright 是 42,512
> 筆，兩套都能從搜尋結果打開完整時間軸。`ostracon export` 沒有伺服器可按需產生
> timeline，所以靜態站台只搜尋這趟確實匯出的子集，頁尾會明示涵蓋率。
...
只有 4.0% 的 commit 說得出為什麼，把空格填滿或摺疊掉都是對資料說謊。標頭把
「相異的專屬／整批引文群組」與可跳轉的「理由列游標」分開，扇出不再被重複算成
很多條理由。
...
**方向是低估的**：這會**過度**正規化，也就是把「其實有差異」判成「只是改名」，
結果是 `change_level` 偏小，hotspots 可能少算一次結構改動；不會反過來把純格式化
灌成 shape。身份匹配仍要通過雙端 bucket 唯一性，不能只靠這個 hash 任意配對。
