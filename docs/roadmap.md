...
- **W7 release candidate**：✅ 已完成。功能凍結，只做壓力測試與邊界釐清：
  **angular/angular**（38,278 commit、292 萬 revision）跑完並分段量測，把
  「成本非線性」定位成**兩顆大規模 revert 佔全程 52%**，而不是隨資料量惡化；
  補上**全 repo pass 的逐 commit 檢查點**（不變量 12 的「可恢復」原本只兌現一半，
  資料逐 commit 落地但水位線整趟才寫）；`--until` 分段索引寫進文件。
  期間更正了自己發布過的一個數字——8/24 那一列是被中斷那趟的部分結果。
- **W8 發布**：✅ 已完成。`ostracon@0.1.0` 發布於 2026-08-30，tag `v0.1.0`
  與 GitHub Release 都指向同一顆 commit（`origin/main`、tag、npm 的 `gitHead`
  三處一致，tarball shasum 與發布前 dry-run 逐字相同）。
  效能專題也完成了，**但原本的題目是錯的**（見下）。
  發布前的 dry-run 抓到一個 CI 看不見的錯配：本機 `.npmrc` 指向鏡像登錄檔，
  而 `package.json` 沒有 `publishConfig`——**打包那條路徑驗得很紮實，發布那條
  一次都沒走過**。
- **W9 前端遷移與交付補強**：✅ 已完成。舊的 579 行樣板頁面已刪，React／Vite
  工作台完成三次線上驗收與兩次回滾；資產／資料路徑、空／非空清單、正式／測試
  CSS 來源等「同一件事兩種執行方式」都補上對應守門。前端從零掛載測試起步，
  先建立 14 條真實渲染契約；後續 W10 擴成 33 條。修補分別隨 0.1.1、0.1.2
  抵達 npm。
- **W10 理由份量與可達性**：✅ 已完成。以 pypa/pip 與 microsoft/playwright
  兩套陌生 repo 定規格：理由改以引文群組與 reach 呈現、宣告分成 Best explained／
  Most changed 兩種排序，且本機 UI 可搜尋全部 indexed 宣告。過程發現並修正
  repo-scoped 索引再跑 `why` 會分裂 entity 身份的缺陷；兩套語料與三套 demo 都以
  fresh DB 驗收。`ostracon@0.1.3`、annotated tag 與 GitHub Release 已於
  2026-09-07 完成，三者指向同一顆 `5b926c5`。後續的抽取器 0.6.0、全域搜尋入口、
  手機時間軸與視覺收斂已隨 `ostracon@0.1.4` 於 2026-09-10 發布；npm、tag 與
  GitHub Release 都指向 `51f1f1f`。

### 下一個決策閘門

跨句與孤兒括號的 197 組穩定鍵已逐條裁決（見 `plan-quote-boundaries.md`）：廣泛句點
截斷只對 16.5%，廣泛括號左擴只對 28.6%，兩者都否決。窄的時間義
`since Python <版本><句末標點>` 已進抽取器 0.6.0；它在完整可見資料中命中 9 種
引文／10 個來源 occurrence，全部是時間義，且不碰 4 種真正的因果用法。

0.1.3 的全域 declaration 入口已觀察並完成；第二輪任務式觀察找到的手機
Evidence 不可見也已修正。重發 demo 後，同三條任務在線上 390 px 畫面全部通過：
已知宣告可直達逐字引文、`hasBit` 的誠實留白可讀、Hotspots 的陌生探索與返回完整。

視覺 polish 也已守住界線完成：只收斂選中列、disabled 狀態、手機欄名與引文的
層級／字級／留白，不改資料契約、排序、抽取或導覽。`ostracon@0.1.4` 已走完版本、
封裝、registry dry-run、正式發布與封裝外重裝；207 檔的 registry shasum 與 dry-run
...
成長**，於是這一節原本規劃的是「分塊比對」——一個會動到匹配器、**會影響正確性**
的改動。

**那個成因是推論，而且用當時手上的資料就能否證**：那兩顆 commit 的匹配工作是
**零**（全是新增，前像池空），而匹配量最大的兩顆只花一分鐘上下。

真正的成因是 `previousPathEntity` 少一條索引而全表掃描 `revision`，且它
「每一次誕生呼叫一次」。修法是 `idx_revision_path`，單次 1,316 ms → 0.027 ms；
`why` 的主查詢 `entitiesFor` 吃同一條索引的前綴，1,883 ms → 8.1 ms。
詳見 `status.md`。

**分塊比對因此不做。** 它要優化的東西不是瓶頸，而它的代價（限制 L5 看得到的
範圍）是真的。這一節留著是為了記住教訓：**先量出成因再選題目**，
否則會花一整個專題去優化一個不存在的瓶頸——而且是往正確性有風險的方向。
...
| 同上，但 3.8 萬 commit ／ 292 萬 revision | 實測 **48 分鐘**、DB 1.4 GiB（`idx_revision_path` 修好之前是 185 分鐘，其中 96 分鐘是兩顆 commit） |
| 單一 commit 碰到上千個檔案 | 已不是特例成本：3,015 個檔的 revert 由 69 分 35 秒降到 45 秒以下 |
...
| **angular/angular** | **38,278** | **2,919,032** | **76.3** | **2,877** | **75.2** | **0.99** | **0.50** |

**angular 那一列不要拿來擬合，它是用來否證外推的。** 兩項模型（用 nest 與 vue 解）
對它預測 29.5 分鐘，實測 48 分鐘。**那一列原本是 185 分鐘／3.88 ms per revision**，
`idx_revision_path` 修好之後才落到現在的值；前五套語料逐項不變。
...
**一次重新加回一千多個 TS 檔的大規模 revert**。成因後來查清楚了，**不是候選池**
（那兩顆的匹配工作是零），是一條缺索引的查詢——**成本的單點是「最大的那顆
commit」，不是「累積了多少資料」**。
...
（那 96 分鐘後來查清楚是缺索引造成的，修好後全量 48 分鐘、低估倍率 1.6 倍。）
...
7. ~~**單一 commit 碰到上千個檔案時，那一顆可能要數十分鐘**~~ ✅ 已修
   （2026-08-30）：成因不是候選池，是 `previousPathEntity` 少一條索引而全表
   掃描 `revision`，且它「每一次誕生呼叫一次」。加 `idx_revision_path` 之後
   單次 1,316 ms → 0.027 ms。前後對照見 `status.md`。
