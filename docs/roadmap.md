...
- **W8**：發布 + 修爆掉的東西。**另加一個效能專題**（見下）。

### W8 的效能專題：巨型 commit 的候選池

W7 定位出來但**刻意沒做**的東西，理由是 W7 說好凍結功能而它會動到匹配器。

一個 commit 的候選池是**它碰到的所有檔案的宣告**，相似度比對隨池子大小超線性
成長。angular 的兩顆 revert（1,637 與 3,015 個檔）因此各花 26:49 與 69:35。

要做的話，先量再改，而且要回答兩個問題：

1. **分塊比對會不會改變配對結果？** L5 的價值就是跨檔案候選池；把池子切開等於
   限制它看得到什麼。任何分塊策略都必須先證明黃金測試集不退步——這不是效能
   改動，是**會影響正確性的改動**。
2. **值不值得？** 五套語料裡只有 angular 撞到，而且是兩顆 commit。
   在「不做非目標清單上的東西」這條規則下，它得先證明自己不是特例優化。
...
| 單一 commit 碰到上千個檔案 | **數十分鐘**——目前最大的單點成本，`--until` 分段切不開它 |
