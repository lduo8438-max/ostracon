...
層級／字級／留白，不改資料契約、排序、抽取或導覽。下一個閥門是準備 0.1.4，
把已完成的前端修正與抽取器 0.6.0 一起走完整的版本、封裝與 registry dry-run；
真正 `npm publish` 仍是獨立的不可逆確認點。第三套陌生 repo 仍不先做。
