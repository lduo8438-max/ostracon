...
0.1.3 的全域 declaration 入口已觀察並完成；第二輪任務式觀察確認搜尋與
Hotspots 都能抵達正確時間軸，新的最高優先級是手機上看不到 Evidence 欄。
920 px 以下已改成直排 revision 卡片，並把隱晦的計數捷徑改成
**Next direct rationale**。下一個閥門是先重發 demo、重跑同三條任務；
只有這條路徑穩定之後才做字體、留白、色彩與動效的純視覺 polish。
第三套陌生 repo 仍不先做。
