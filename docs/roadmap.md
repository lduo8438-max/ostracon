...
- **W1 地基**：✅ 已完成（提前）
- **W2**：意圖層 + 最醜的 UI，端到端必須跑通
- **W3–4 靈魂**：跨重構的實體追蹤做到真的準、被推翻的方案偵測、增量索引
- **W5–6**：issue／PR 連結解析、攪動熱點視圖、成本控制、加 Python 驗證架構
- **W7 發布準備**：一行安裝、**預先索引三到五個知名開源 repo 做成線上 demo**
  （訪客不用裝就能玩）、README、GIF、架構文件
- **W8**：發布 + 修爆掉的東西
...
3. **span 斷言機制**（✅ 完成）：只用 commit message，抽取式，程式驗證 span 確實
   ...
   **還沒做**：真正產生候選的抽取器。目前只有測試用的規則式候選（取第一行）。
   接 API 之前，這道驗證已經可以獨立擋住幻覺。
4. 才是意圖層與最醜的三欄 UI。
...
1. **平行分支上的血緣**：維護的是全域 path → lineage 對照表而非逐 commit 樹狀態。
   ...
   成本高一到兩個數量級。受影響嚴重的 repo 可用 `--first-parent` 完全避開。
