...
0.1.3 的第一輪真實使用觀察已完成。最高優先的摩擦不是搜尋品質，而是 declaration
入口藏在 Timeline：既有 picker 已提升成所有畫面都看得到的全域動作，按鈕與 `/`
共用同一路徑。下一個候選是術語的就地解釋；先觀察這個入口是否足以縮短首次抵達
literal evidence 的路徑，再決定是否做。mobile 三欄重排、第三套陌生 repo 與純視覺
polish 仍不先做。
