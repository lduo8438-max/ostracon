...
**2. `createGitHubFetcher` 沒有任何重試。** 第一次跑 create-t3-app 時，
第一個請求就遇到 TLS `ECONNRESET`，整趟直接拋例外中止。per-commit transaction 與
水位線讓它可以續跑（沒有資料損壞），但一趟 3,000 請求、40 分鐘的執行不該被
單次網路抖動打斷。rate limit 已經有優雅的 `stopped` 路徑，暫時性網路錯誤沒有。
量測時是在 runner 端補了指數退避重試，**產品程式碼未動**。
