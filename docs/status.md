...
共用引文數保留為次要訊息。重發 demo 後，同一條第 25 列的逐字引文已在線上
390 px 畫面出現在選中卡片內；`hasBit` 的同日生滅與誠實留白可讀，從 Hotspots
陌生探索再返回清單也完整。三條任務的瀏覽器 console 都是 0 錯誤。

功能路徑穩定後才做視覺收斂：選中列改成冷色細邊與淡底，不再用整片藍底和唯一的
暖色逐字引文搶層級；無直接理由的 disabled 控制改為中性狀態；手機欄名、metadata
與引文字級也提高。390 × 844 無橫向溢出，1440 × 900 保留原三欄對齊。修法拿掉時
新增的掛載式與 CSS 契約都會紅；目前完整核心測試 **487/487**、前端契約
**35/35**，typecheck 與正式 build 通過。
...
137 條的後句仍屬同一理由；孤兒括號 28 條只有 8 條需要左擴，19 條是應忠實保留的
括號子句，另 1 條只是 `1)` 清單誤判。因此不做「第一句截斷」、不做廣泛補括號，
也不在呈現層刪逐字標點。

裁決另外找出一條窄規則：`since Python <版本>` 後立刻接句末標點的 9 種相異引文
（10 個來源 occurrence）全是時間義，而 4 種真正的 `since Python` 因果用法都有
不同句法。它已進抽取器 `rule-rationale-0.6.0`，四種因果反例由測試守住。

兩套語料的只讀反事實重抽中，pip 移除 10 個舊鍵並恢復 1 條原先被吞掉的真正理由，
playwright 的 684 個鍵逐一相同；在暫存副本走真實舊版本作廢／重抽後，pip 與
playwright 分別升格 987／684 筆，兩者 rejected 都是 0。完整方法、成本與候選指紋
見 `plan-quote-boundaries.md`。
...
### 讓稀有的理由被找得到：意圖跳轉（2026-08-30）

發布後第一次以使用者身分讀 demo，抓到一個**資料正確但找不到**的缺口：
vue 的 `compileScript` 左欄明示「2 entity-specific rationales」，選進去之後
右欄首屏全是短橫——因為那兩條理由在 **306 列時間軸的第 25 列與第 232 列**。

稀疏是刻意的（理由本來就稀有，Osiris 4.0%），**但「稀疏」與「找不到」是兩件事**。

#### 為什麼是可點的計數，不是篩選模式

考慮過「只顯示有理由的列」這種篩選。**否決的理由是架構上的**：整個畫面建立在
「每一次改動一格、沒證據就留白」，而 `alignRows` 逐列量測把意圖欄第 n 格對齊
演化欄第 n 列。那段註解裡的話是這個檔案最重的一句——

> 對齊錯了就是把 A 的理由印在 B 底下，而那是這個工具最不能犯的錯。

篩選會摧毀那個一對一關係：要嘛破壞對齊，要嘛得寫第二條對齊路徑，而那是
「同一件事兩份實作」的形狀。捲動則完全不用碰對齊，雙向同步已經在了。

所以標頭那個計數**本身就是控制項**：affordance 與事實是同一個物件，
不需要解釋一個新模式。點「2」依序跳到那兩列，循環；「3 batch-only」是另一個
按鈕，獨立巡覽且視覺次一階（延續「唯一的暖色只給引文」）。
**不填滿、不摺疊任何空白。**

#### 標頭的計數與跳轉清單必須是同一個陣列

分開算的話標頭會說 2、按鈕卻跳三個地方。這個專案被同型的分岔咬過一次
（CLI 說 5 顆聚合 commit、UI 說 6 顆，因為畫面另寫了一份 SQL），所以
`rationaleTargets` 一次算出兩組索引，計數與巡覽都用它。

#### 深連結延伸，舊形式必須繼續有效

片段從 `#<stable_key>` 延伸成可選的 `#<stable_key>/<shortSha>`。
**舊形式已經公開在 demo 的 landing 上**，而網址是使用者存起來、貼給別人的
東西——加後綴不能讓既有連結失效。分隔用斜線是安全的：寫入端一律
`encodeURIComponent`，資料裡真正的斜線會變成 `%2F`。

#### 讓頁面執行的就是被測試的那一份

頁面是一整個樣板字串，裡面的 JS 只能用 regex 比對，而 regex 咬不住邏輯。
所以三個純函式放進 `src/ui/page-logic.ts`，用 `toString()` 注入頁面，
並有一條測試釘住「頁面裡沒有另一份抄本」。
（`--experimental-strip-types` 把型別換成空白而不是刪除，所以 `toString()`
拿到的仍是合法 JS——這一點是先量過才敢用的。）

與今天早上消滅 `previousPathEntity` 的 SQL 抄本同一個做法：
**與其把比對寫嚴，不如讓副本不存在。**

#### 補上一道先前完全沒有的守門

加這個功能時**三次踩到樣板字串的逸出陷阱**，其中一次是在寫那個陷阱的註解時
（註解裡的插值起頭是活的）。三次都剛好讓 `tsc` 報錯，**但只要逸出「剛好」湊成
合法的 TS，錯就會靜默地送到瀏覽器**——`tsc` 只檢查得到它是不是合法字串。

所以補了一條：把頁面的 script 抽出來 `new Function` 編譯，語法錯就紅。
驗過會咬。

同時**刪掉一條我自己寫錯的測試**（「成品裡不該出現插值語法」）：頁面故意有
巢狀樣板字串給瀏覽器求值，那條會誤報正確的程式碼。寫完當場紅，當場撤。

#### 既有測試咬到了一次，而它咬的是拼法

`ui.test.ts` 原本斷言 `history.replaceState(null, "", "#" + encodeURIComponent(...))`
這一行的**字面寫法**。改走 `formatTimelineHash` 之後行為不變（仍是 replaceState、
仍以 stable_key 為鍵）但拼法變了，於是它紅。改成釘不變量：有沒有走那個唯一的
入口，以及頁面不得自己拼網址。

#### 驗收

測試 435 → 447。四條咬檢各自驗過：舊連結失效、計數與清單分岔、頁面自己抄一份、
命中列改用會影響盒模型的屬性。

Chrome 實測由作者完成並通過六項（兩次跳轉與循環、網址同步、帶 sha 的冷開深連結、
不帶 sha 的舊連結、以及**視覺對齊沒有跑掉**）。命中列用 inset box-shadow 而不是
邊框或內距，就是因為後兩者會改變列高——測試釘得住 CSS 屬性，但錯位只有眼睛
看得出來。

**留著一個已知觀察**：全新分頁第一次載入大型時間軸（306 列）會先空白 3–4 秒。
行為正確，但那段時間沒有任何進度指示。**成因還沒量，所以不寫。**
...
`roadmap` §3／§4 與這份文件。**那句話是推論不是量測，而且用當時手上的資料就能
否證。** W8 的效能專題原本要照著它去做「分塊比對」——那會是**改動匹配器**、
會影響正確性的改動，而它要優化的東西根本不是瓶頸。

#### 一、用同一批資料否證

| commit | 檔案 | revision | births | deaths | match 列 | 耗時 |
|---|---:|---:|---:|---:|---:|---:|
| `66f49c24` | 1,637 | 1,092 | 1,092 | 0 | **0** | 26:49 |
| `e41a5228` | 3,016 | 2,381 | 2,381 | 0 | **0** | 69:35 |
| `3bbd12d5` | 17 | 5,013 | 617 | 687 | **4,362** | 1:08 |
| `4f94bd78` | 5 | 5,076 | 10 | 3 | **5,063** | 0:48 |

**兩顆病理 commit 的匹配工作是零**——全是新增，前像池是空的，階梯無事可做。
而匹配量最大的兩顆（各四千、五千次配對）只花一分鐘上下。候選池假說預測的排序
與實測相反。

三個候選量都對不上，只有一個對得上：

- 依 **TS 檔數**：1,092 → 1,268 是 1.16 倍，耗時卻是 2.59 倍。要 6.4 次方才吻合。
- 依 **插入行數**：80,982 比 90,158 **更少**，耗時卻更多。方向相反。
- 依 **總檔案數**（含 grammar 不認得的）：1.84 倍 → 約 1.6 次方。形狀對得上，
  但匹配器根本不看那些檔案。
- 依 **誕生數 × 當下 `revision` 列數**：兩顆的比值 2.46，實測 2.59。**吻合。**

#### 二、真正的成因

`previousPathEntity`（路徑被刪除後又重建時，找回前一位佔用者）在 `revision` 上
**沒有可用的索引**，於是全表掃描；而它是「**每一次誕生呼叫一次**」。
大規模 revert 的每一列都是誕生，所以那一顆 commit 會把整張表掃上千遍。

對照組就在隔壁：`previousSlotEntity` 是同一張表、同一種形狀的查詢，
有 `idx_revision_slot`。差別只在有沒有索引。

| | 查詢計畫 | 每次 |
|---|---|---:|
| `previousPathEntity`（修前） | `SCAN r USING INDEX idx_revision_entity` | **1,316 ms** |
| `previousPathEntity`（修後） | `SEARCH r USING INDEX idx_revision_path` | **0.027 ms** |
| `previousSlotEntity`（對照） | `SEARCH r USING INDEX idx_revision_slot` | 0.002 ms |

修法是一條索引：`idx_revision_path ON revision(repo_id, lineage_id, path)`，
建立耗時 1.8 秒（290 萬列）。

#### 三、順手修好 `why` 的主查詢

同一條索引的前綴 `(repo_id, lineage_id)` 正好是 `entitiesFor` 的過濾條件——
那是 `why` 每次呼叫都要跑的查詢，先前也在全表掃描：
**1,883 ms → 8.1 ms**（angular，290 萬列）。

這一條不是巧合，是同一個成因的第二個受害者：`revision` 上原本只有
`entity_id`、`slot_id`、`content_id` 三條索引，**沒有任何一條以血緣或路徑為鍵**，
而「這條路徑上有什麼」正是這個工具最常問的問題。

#### 四、量測方法上的一次自我更正

掃描稽核順手發現跨 repo 守門（`assertNoCrossRepoRows`）也全表掃描，於是量了
加索引前後——第一次的結論是「索引害它從 159 ms 變成 4,789 ms，慢 30 倍」，
並據此加了 `NOT INDEXED`。

**那是量錯的。** 兩個數字來自**不同檔案**（`main.db` 與加了索引的副本），
而前者在那之前已被反覆讀取、頁面都在快取裡。同檔交錯重測（兩種順序各五輪，
背景負載相同）的結論相反：

| | 中位數 |
|---|---:|
| 吃索引 | **1,471 ms** |
| `NOT INDEXED` | 2,416 ms |

`NOT INDEXED` 已撤回。留下的教訓有兩條：**跨檔案比較效能等於在量快取狀態**；
以及「加索引只會變快」不成立——它確實會改變其他查詢的計畫，只是這一次沒有。

#### 五、schema v3：第一個可就地遷移的版本

索引不改變任何資料，**產出逐位元不變**，而要求使用者重建的代價是 angular
三小時。所以 v2→v3 是遷移不是拒絕。`MIGRATIONS` 是白名單而不是通則——
v1→v2 改了資料的存法，那一段仍然拒絕。**判準是「產出會不會變」，
不是「版本號有沒有變」。**

#### 六、實跑對照：angular 全量重建

指紋比對只證明「沒有算錯」，效能要另外實跑。同一台機器、同一份語料、同一支
`profile:angular`，重建整個 angular（8/27 那趟的原始紀錄在
`docs/benchmarks/angular-20260827/main.jsonl`）：

| 區間 | 修前 | 修後 | | 修前最慢的一顆 | 修後最慢的一顆 |
|---|---:|---:|---:|---:|---:|
| 0–5k | 4.89 分 | 3.93 分 | ×0.80 | 17.4 s | 4.0 s |
| 5k–10k | 5.18 分 | 4.14 分 | ×0.80 | 22.8 s | 5.4 s |
| 10k–15k | 7.72 分 | 4.71 分 | ×0.61 | 94.2 s | 38.5 s |
| 15k–20k | 10.88 分 | 10.04 分 | ×0.92 | 67.8 s | 48.0 s |
| **20k–25k** | **62.27 分** | **4.06 分** | **×0.07** | **1,609 s** | **45.6 s** |
| **25k–30k** | **75.32 分** | **4.15 分** | **×0.06** | **4,175 s** | **44.8 s** |
| 30.7k／30k–35.7k／35k | 8.03 分 | 6.22 分 | ×0.77 | 47.9 s | 45.9 s |
| 35.7k／35k–38.3k | 11.10 分 | 10.12 分 | ×0.91 | 37.6 s | 16.0 s |
| **全程（pass 加總）** | **185.39 分** | **47.38 分** | **×0.26** | | |
| **全程（牆鐘，含 git 走訪）** | — | **47.96 分** | | | |

最後兩列的區間對不齊：**修前那趟因為中途續跑，分段點落在 30.7k／35.7k**，
與這趟的 30k／35k 差約 700 顆 commit，所以那兩列只能粗比。
20k–25k 與 25k–30k 兩列的分段點是對齊的。

**兩顆病理 commit 一起消失了。** 修前最慢的兩顆是 26:49 與 69:35；修後最慢的
一顆是 48 秒，而且分佈平坦——沒有任何區間再由單一 commit 主導。

8/27 標為「形狀不同、沒查清楚成因，所以不寫」的那顆 94.2 秒 `147a15a4`
**是同一個成因**：單一新檔，但有 117 個誕生。當時不寫成因是對的。

一般區間也快 8–20%，因為誕生在每顆 commit 上都有，只是量少。

#### 七、產出在 292 萬列上逐位元相同

比指紋更強的一次確認：新舊兩份完整索引的五組指紋全量比對。

| | 列數 | 摘要 |
|---|---:|---|
| `revision` | 2,919,032 | `d6cf944650fe9df2` |
| `revision_change` | 2,815,574 | `f7370e41d210e029` |
| `revision_match` | 2,800,838 | `92bbe00ffd4d4b48` |
| `entity`（含生死 commit） | 118,194 | `3608bf1dd2644ae4` |
| `slot_discontinuity` | 8,739 | `2dc5137c36e96bc0` |

**五組全部相同**，兩條水位線都在 topo 38,277，`quick_check = ok`。

而且對照的那一份是**分段、經歷兩次真實中斷續跑**建出來的，這一份是
「一趟跑到 25k，再從水位線續跑到 HEAD」——所以它同時第三次複驗了檢查點。

#### 八、代價：索引佔 16.3% 空間

不是免費的。同一份完整 angular 索引：**1,230.0 MiB → 1,430.4 MiB（+16.3%）**。
每 revision 的體積由 0.40 KB 變成 0.49 KB。
（在既有資料庫上補建這條索引是 +178.7 MiB／+14.5%，差別是頁面碎片。）

峰值 RSS **3,471 → 3,976 MiB（+14.5%）**，而且**方向隨規模改變**：
topo 5k／10k 時是 2,651 → 2,390 MiB（更低），20k 之後才反超。

**這個 RSS 對照有一個沒排除的干擾**：新這趟設了 `--max-old-space-size=8192`，
8/27 那趟沒有（工具與紀錄裡都沒有這個旗標）。早期反而更低這件事讓「旗標把記憶體
撐大」說不通，而 +14.5% 與磁碟開銷同比例、和「索引頁進了快取」一致——
但要乾淨歸因得再跑一趟受控的。**沒排除之前不把 RSS 的變化算到索引頭上。**

#### 九、遷移在**真實的 v2 資料庫**上驗過（2026-08-30 補）

上面那三條測試用的是**合成的 v2**：建一個現行版本的庫，再拿掉索引、把版本改寫
成 2。那是論證不是實測——「合成的等於真實的」這句話本身就是這個專案被咬過
十四次的形狀。

先確認結構上真的等價：`git diff` v2→v3 的 `db/schema.sql`，**唯一的非註解變更
就是那一行 `CREATE INDEX`**。然後在真實的 v2 上跑一次端到端：

| | |
|---|---|
| 素材 | `reports/demo-osiris.db`，真實 v2（`schema_migration` 只有版本 2、無索引） |
| A | 原封不動，`ui:export` 匯出靜態站台 |
| B | 先 `openIndexDatabase`（觸發遷移），再匯出 |
| 結果 | **311 個檔案逐檔零差異**；遷移耗時 6 ms；`schema_migration` 變成 2、3 兩列 |

**另外兩條恢復性質也驗了，其中一條補成測試：**

1. **冪等**：已是 v3 再開一次不會重複插入版本列（`found === SCHEMA_VERSION`
   直接返回）。
2. **遷移中途被中斷**：建索引與寫版本列**不在同一個 transaction**，死在兩步
   之間的資料庫是「索引在、版本還是 2」。接得回來靠的是
   `CREATE INDEX IF NOT EXISTS`——重跑第一步是 no-op。拿掉那三個字，續開會炸在
   「索引已存在」，而使用者手上會是一個開不起來、又不該重建的資料庫。

第 2 條寫成測試時**第一版是錯的**：全新建的庫只有版本 3 那一列，把它刪掉會變成
「沒有版本紀錄」，那是另一條路徑。要造出「索引在、版本 2」得改寫而不是刪除。
**是這條測試自己咬出來的**——手動驗證時我用的是真實 v2 遷移後的檔案（有 2、3
兩列），刪掉 3 剛好留下 2，所以矇對了。

測試 434 → 435。

#### 十、驗收

- **輸出不變**：osiris 前後 `revision`／`revision_change`／`revision_match`／
  `entity`／`slot_discontinuity` 五組指紋逐位元相同。
  **選 osiris 是因為只有它走得到這條查詢**——instrument 之後
  create-t3-app／requests／vue-core 都是 0 次，osiris 12 次。
  在沒走到的語料上比對指紋，比的是兩份沒有差別的東西。
- 五套 golden 全過，測試 431 → 434。
- 三條新測試各自驗過會咬：拿掉 schema 的索引 → 計畫斷言紅；
  拿掉遷移 → 遷移斷言紅。

**計畫斷言不是「索引存在」而是「`revision` 沒有被 SCAN」**，而且查詢是從
`structural.ts` 匯入的。第一版在測試檔裡自己抄了一份 SQL，「拿掉機制看會不會紅」
時發現：在查詢尾端追加條件時抄本比對不會咬。與其把比對寫嚴，不如讓副本不存在。
...
8/24 寫進 README 與 roadmap 的 angular 那一列是
`1,697,002 revision / 44.3 rev/commit / 239 分鐘`。**那來自被中斷的第一趟，
只有 58% 的 revision。** 時間我當時標了「是下界」，**但 revision 數沒有標**——
等於把一個部分結果當成總量發布。完整的數字是：

| | 錯的（部分） | 正確（完整） |
|---|---:|---:|
| revision | 1,697,002 | **2,919,032** |
| rev/commit | 44.3 | **76.3** |
| 時間 | 239 分鐘（未完） | **185 分鐘（跑完）** |
| DB | 768 MB | **1.20 GiB** |
| 峰值 RSS | 3,479 MiB | 3,471 MiB |

`declaration_content` 276,161 列 = revision 的 **9.5%**，與 vue 的 9.8% 一致
——內容定址的去重率在這個規模上沒有退化。`quick_check = ok`，WAL 已歸零。

#### 二、非線性不是「隨資料量惡化」，是兩顆 commit

| 區間 | 耗時 | revision | 該區間最慢的一顆 |
|---|---:|---:|---|
| 0–5k | 4:53 | 437,681 | 17.4 s |
| 5k–10k | 5:11 | 432,519 | 22.8 s |
| 10k–15k | 7:43 | 363,840 | 94.2 s |
| 15k–20k | 10:53 | 375,301 | 67.8 s |
| 20k–25k | **62:16** | 278,787 | **26:49** |
| 25k–30k | **75:19** | 307,834 | **69:35** |
| 30.7k–35.7k | 8:02 | 398,338 | 47.9 s |
| 35.7k–38.3k | 11:06 | 272,318 | 37.6 s |

那兩顆是：

- `66f49c24` — `Revert "refactor: remove checked-in locale files"`，
  **1,637 個 TS 檔、90,158 行插入**，26 分 49 秒。
- `e41a5228` — `Revert "refactor: move example generation into adev"`，
  **3,015 個檔（1,284 個 TS）、80,982 行插入**，69 分 35 秒（佔它所在區間的 92%）。

**兩顆合計 96.4 分鐘 = 全程的 52%。** 扣掉之後其餘 38,276 顆是 89 分鐘、
1.86 ms/revision，而一般區間只從 4:53 溫和成長到 11:06。

**所以 8/24 寫的「成本在這個規模上不是線性的」要修正**：整體趨勢是溫和的，
非線性集中在少數病理 commit。

（當時這裡接著寫「成因也清楚了——候選池隨池子大小超線性成長」。**那是推論，
而且是錯的**，2026-08-30 用同一批資料否證了它，見下一節。）

**成本的單點是「歷史上最大的那顆 commit」，不是「累積了多少資料」。**
這改變了該告訴使用者什麼：先前寫「數萬 commit 的 repo 跑不完」，現在該問的是
「你的歷史裡有沒有一次動到上千個檔案的 commit」。

（10k–15k 那顆 94 秒的 `147a15a4` 是單一 2,940 行的新檔，形狀與那兩顆不同，
**沒有查清楚成因，所以不寫**。）
...
1. **`database is locked`**——續跑程序在原程序還活著時啟動，被 SQLite 拒絕。
   supervisor 確認原程序仍在 99.4% CPU 上跑，判定「拒絕是對的」。
   **那不是缺陷，是併發寫入被正確擋下**，而且是我設計檢查點時沒想到的一種情況。
2. **SIGTERM 刻意暫停**（topo 30,701，`quick_check = ok`），兩小時後續跑，
   19 分鐘跑完剩下的 7,576 顆。

**最終 `structural` 與 `declarations` 兩條水位線都抵達 topo 38,277 = HEAD。**
另外還有一次刻意中止的閘門測試（`gate.db`：在 topo 4,999 中止、續跑到 10,000、
exit 0）——那是 8 個 commit 的單元測試之外，第一次在真實大語料上驗證續跑。

### 全 repo pass 的檢查點（2026-08-24，已修）

angular 那一輪的第二個發現。**資料是逐 commit 提交的，水位線卻只在整趟迴圈跑完
的最後一行寫入**——中斷之後 `pass_state` 沒有 declarations 那一列，
`resolveResumePoint` 判成 `full`，下一趟從第一顆 commit 重來。

那段 transaction 的註解本身就寫著設計意圖：

> 邊界取在單一 commit 而不是整趟：整趟包成一個的話，一萬個 commit 的 WAL 會
> 膨脹到不可接受，**中途失敗也會把已完成的工作全部丟掉**。

**意圖只兌現了一半。** 資料的確保住了，但「已完成」這件事沒有被記錄下來，
所以保住的資料接不回去。angular 上那是 170 萬筆 revision、四小時。

修法是把水位線寫進**同一個** per-commit transaction。放在 transaction 外面不行：
程序死在兩者之間會讓水位線超前資料，那比不前進更糟。

**每顆都寫，不設每 N 顆的門檻。** 它是一次 UPSERT，而那個 transaction 本來就要
提交。實測開銷在雜訊內：

| 語料 | 無檢查點 | 有檢查點 |
|---|---:|---:|
| create-t3-app | 8.58 s | 8.61 s |
| psf/requests | 100.39 s | 99.30 s |

沒有可調的門檻，就沒有調錯的門檻。

#### 測試：第一版是假閘門，當場拆掉

第一版斷言「跑完之後水位線指向最後一顆處理過的 commit」——**而整趟結尾那次寫入
也滿足它**，拿掉迴圈內的檢查點照樣全綠。那是這一段第二次自己造假閘門了。

改成**真的在迴圈中途打斷**：在 `db` 這個既有的接縫上包一層 Proxy，數
`INSERT INTO revision` 的 `run` 次數，第 4 次丟例外。產品程式碼一行沒動，
而那是一次發生在 transaction 內的中斷，與 angular 上被訊號砍掉的形狀相同。

（Proxy 有個坑：`Reflect.get` 回傳未綁定的原生方法，用 Proxy 當 `this` 呼叫
`db.exec` 會得到 `Illegal invocation`。前提檢查先報「注入沒有觸發」才查出來的。）

斷言四件事，缺一不可：中斷之後水位線仍在、它指的 commit 真的有資料、它停在
中斷之前（不是 tip）、水位線之後沒有任何已寫入的 revision（炸掉那顆整個回滾）。
**再接著跑一次並與一次跑完的產出逐項比對**——水位線留下來只是必要條件，
使用者要的是「中斷之後再跑，拿到的跟沒中斷過一樣」。

拿掉檢查點之後這條測試會紅。測試 430 → 431。
...
第三刀的 CPU profile 指出 pw 的大宗成本不是 SQL：`lineRange` 自身 457 s（repo pass
的 27%）、`utf8ByteRange` 103 s，而全部 SQL 只有 124 s。兩者都用
`source.slice(0, index)` 從檔案開頭重算，成本是「宣告數 × 檔案大小」；
**自 initial commit 就是這樣，與 v5 無關**，所以獨立一刀。

`createSourcePositions` 每份原始碼建一次表（行首位移，加非 ASCII 時每行的 UTF-8
位元組前綴），宣告共用；查詢是二分搜尋加「行首到該位置」的一小段。純 ASCII 檔案
位元組位移恆等於字元位移，連表都不查。

**結束行仍由節點文字的換行數推導**，不改成第二次查表：那會把定義從「宣告佔幾行」
換成「endIndex 落在第幾行」，兩者在文字以換行結尾時不同，而它是 `revision` 的欄位。

| playwright，同機無插樁 | 修前 | 修後 |
|---|---:|---:|
| repo pass | 1,724.4 s | **1,069.8 s（×0.62）** |
| 峰值 RSS | 2,469 MiB | 2,250 MiB |
| 十張身份表指紋 | — | **全部相同** |
| `revision` 的 line_start/line_end、byte_start/byte_end（234 萬列） | — | **逐列相同** |

修前那一趟前半段有其他負載，所以也對乾淨的 v4／v5 插樁基準比一次：1,672／1,681 s
→ 1,070 s（×0.64）；第 6,000 顆檢查點 356／358 s → 262 s。

**非 ASCII 路徑在真實資料上驗過**：自我索引（註解全是中文）前後十張指紋相同，
且依資料庫記的位元組位移從 blob 切出來的 3,998 筆 revision 全部能乾淨解碼、
起始行號與實際位置相符。

`test/positions.test.ts` 對照被取代的樸素實作：八種形狀（ASCII／中文／代理對
emoji／CRLF／無尾端換行／連續空行／單行／空檔）的**每一個字元位移**逐點比對，
外加固定種子的隨機原始碼 40 輪。三種破壞各自驗過會紅：ASCII 快路徑恆真、
